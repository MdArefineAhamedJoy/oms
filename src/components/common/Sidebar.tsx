...
const ALL_ITEMS = [
  ...
  { label: "Roster", href: "/admin/roster", icon: CalendarCheck, roles: ["admin"] },
  { label: "Shifts", href: "/admin/shifts", icon: ListChecks, roles: ["admin"] },
  { label: "Salary", href: "/admin/salary", icon: Wallet, roles: ["admin"] },
  { label: "Attendance", href: "/admin/attendance", icon: UserCheck, roles: ["admin"] },
  { label: "Leave", href: "/admin/leave", icon: CalendarRange, roles: ["admin"] },
  { label: "Reports", href: "/admin/reports", icon: FileChartColumn, roles: ["admin"] },
...
export default function Sidebar({ role, title = "" }: { role: Role; title?: string }) {
  ...
  return (
    <aside
      className={cn(
        "fixed left-0 top-0 h-screen flex flex-col border-r bg-white shadow-sm transition-all duration-300 ease-in-out z-40",
        collapsed ? "w-16" : "w-64"
      )}
    >
